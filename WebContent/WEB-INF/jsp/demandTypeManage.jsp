...
<div style="height: 50%;width: 95%; margin:0 auto;">
 <input type="text" id="query" placeholder="请输入需求类型进行搜索" style="width:30.333%" />&nbsp;&nbsp;&nbsp;<button onclick="query()" class="layui-btn layui-btn-normal" style="width:10.333%" ><span class="layui-btn layui-btn-normal">搜索</span></button>
   <input type="button" class="btn btn-primary" data-toggle="modal" data-target="#Revert3" onClick="Values2(0)" value="添加需求类型"/>	
<table class="layui-table" style="border-collapse:collapse;table-layout:fixed;width:500px;margin:auto;"   border="1">
  ...
  <thead>
    <tr>   
      <th width="150px">需求类型ID</th>
      <th width="150px">需求类型名字</th>
      <!-- <th>需求类型父ID</th> -->
  ...
  <tr>        
  <tbody id="ccc">
   <c:forEach items="${Flist}"  var="demandType">     
  <tr>
  <td>${demandType.parameterId}</td>
  <td>${demandType.parameterName}</td>
  <%-- <td>${demandType.pId}</td> --%>
  <td><a class="btn btn-danger" id="yyy" href="#"  onclick="changeState(${demandType.parameterId},${num});return false;" >删除</a></td>
  </tr>
  </c:forEach>
  </tbody>
  ...
  <form action="<%=path%>demandType/addDemandType.action" method="POST">
                                    ...
                                    <div class="modal-content">          
                                      <div class="modal-header" style="text-align:center;"> 
                                       <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>         
                                            <h4 class="modal-title" id="myModalLabel">    添加需求类型  </h4>
                                      </div>                                        
...
function changeState(parameterId,number){
	...
	$.ajax({
		url:"<%=path%>demandType/changeState.action",
...
function nextPage(){
	...
	$.ajax({	
		 ...
		 url:"<%=path%>demandType/selectPage.action",
